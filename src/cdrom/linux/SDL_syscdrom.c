	...
	if(SDL_SYS_CDioctl(cdrom->id, CDROMREADTOCHDR, &toc) == 0) {
		...
		for (i = 0; i <= cdrom->numtracks; ++i) {
			...
			if(SDL_SYS_CDioctl(cdrom->id, CDROMREADTOCENTRY, &entry) < 0) {
				...
				cdrom->track[i].offset = MSF_TO_FRAMES(entry.cdte_addr.msf.minute, entry.cdte_addr.msf.second, entry.cdte_addr.msf.frame);
...
static CDstatus SDL_SYS_CDStatus(SDL_CD *cdrom, int *position) {
	...
	if(position) {
		if(status == CD_PLAYING || (status == CD_PAUSED)) {
			*position = MSF_TO_FRAMES(info.cdsc_absaddr.msf.minute, info.cdsc_absaddr.msf.second, info.cdsc_absaddr.msf.frame);
		} else {
			*position = 0;
		}
	}
...
static int SDL_SYS_CDPlay(SDL_CD *cdrom, int start, int length) {
	...
	FRAMES_TO_MSF(start, &playtime.cdmsf_min0, &playtime.cdmsf_sec0, &playtime.cdmsf_frame0);
	FRAMES_TO_MSF(start + length, &playtime.cdmsf_min1, &playtime.cdmsf_sec1, &playtime.cdmsf_frame1);
#ifdef DEBUG_CDROM
