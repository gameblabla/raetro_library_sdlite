 ...
 */
#define FORMAT_EQUAL(A, B) ((A)->BitsPerPixel == (B)->BitsPerPixel && ((A)->Rmask == (B)->Rmask) && ((A)->Amask == (B)->Amask))
...
#define PIXEL_FROM_RGB(Pixel, fmt, r, g, b) { Pixel = ((r>>fmt->Rloss)<<fmt->Rshift)| ((g>>fmt->Gloss)<<fmt->Gshift)| ((b>>fmt->Bloss)<<fmt->Bshift); }

#define RGB565_FROM_RGB(Pixel, r, g, b) { Pixel = ((r>>3)<<11)|((g>>2)<<5)|(b>>3); }
#define RGB555_FROM_RGB(Pixel, r, g, b) { Pixel = ((r>>3)<<10)|((g>>3)<<5)|(b>>3); }
#define RGB888_FROM_RGB(Pixel, r, g, b) { Pixel = (r<<16)|(g<<8)|b; }
...
#define RGBA_FROM_RGBA8888(Pixel, r, g, b, a) { r = (Pixel>>24); g = ((Pixel>>16)&0xFF); b = ((Pixel>>8)&0xFF); a = (Pixel&0xFF); }

#define RGBA_FROM_ARGB8888(Pixel, r, g, b, a) { r = ((Pixel>>16)&0xFF);g = ((Pixel>>8)&0xFF);b = (Pixel&0xFF);a = (Pixel>>24) };

#define RGBA_FROM_ABGR8888(Pixel, r, g, b, a) { r = (Pixel&0xFF); g = ((Pixel>>8)&0xFF); b = ((Pixel>>16)&0xFF); a = (Pixel>>24); }

#define DISEMBLE_RGBA(buf, bpp, fmt, Pixel, r, g, b, a) do {  \
    switch (bpp) {                                            \
        case 2:                                               \
            Pixel = *((Uint16 *)(buf));                       \
        break;                                                \
...
#define ALPHA_BLEND(sR, sG, sB, A, dR, dG, dB) do { dR = (((sR-dR)*(A)+255)>>8)+dR; dG = (((sG-dG)*(A)+255)>>8)+dG; dB = (((sB-dB)*(A)+255)>>8)+dB; } while(0)
...
/* Don't use Duff's device to unroll loops */
#define DUFFS_LOOP(pixel_copy_increment, width) { int n; for ( n=width; n > 0; --n ) { pixel_copy_increment; } }
