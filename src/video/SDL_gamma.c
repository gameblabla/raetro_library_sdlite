...
#include "../libm/e_sqrt.h"
#include "../libm/e_pow.h"
#include "../libm/e_log.h"
...
static void CalculateGammaFromRamp(float *gamma, Uint16 *ramp) {
	/* The following is adapted from a post by Garrett Bass, March 4, 2000. */
	float sum = 0.0f;
	int i, count = 0;
...
int SDL_SetGammaRamp(const Uint16 *red, const Uint16 *green, const Uint16 *blue) {
	...
	if((screen->flags & SDL_HWPALETTE) == SDL_HWPALETTE) {
		...
		SDL_SetPalette(screen, SDL_PHYSPAL, pal->colors, 0, pal->ncolors);
		return 0;
	}
