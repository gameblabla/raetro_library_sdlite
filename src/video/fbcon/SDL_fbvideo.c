...
static const SDL_Rect checkres[] = {{0, 0, 1920, 1200},  /**< WUXGA */
									{0, 0, 1920, 1080},  /**< 16:9 = 1.7 / 1080p FHD */
									{0, 0, 1600, 1200},  /**< 16 bpp: 0x11E, or 286 / UXGA */
									{0, 0, 1408, 1056},  /**< 16 bpp: 0x19A, or 410 */
									{0, 0, 1366, 768},   /**< 16:9 = 1.7 / Standardized HDTV 720p/1080i displays or HD ready */
									{0, 0, 1280, 1024},  /**< 16 bpp: 0x11A, or 282 / SXGA */
									{0, 0, 1280, 720},   /**< 16:9 = 1.7 / 720p HD/WXGA */
									{0, 0, 1152, 864},   /**< 16 bpp: 0x192, or 402 / XGA+ */
									{0, 0, 1024, 768},   /**< 16 bpp: 0x117, or 279 / XGA */
									{0, 0, 960,  720},   /**< 16 bpp: 0x18A, or 394 */
									{0, 0, 960,  544},   /**< 5:4 / Playstation Vita */
									...
									{0, 0, 600,  480},   /**< 5:4 / Some older monitors */
									{0, 0, 512,  384},   /**< 4:3 = 1.3 */
									{0, 0, 480,  320},   /**< 3:2 = 1.5 / HVGA */
									{0, 0, 480,  272},   /**< 16:9 / WQVGA Sony PlayStation Portable, Neo Geo X */
									{0, 0, 320,  240},   /**< 4:3 = 1.3 / QVGA */
									{0, 0, 320,  224},   /**< 3:2 = 1.5 / Sega Nomad, Neo Geo AES */
									{0, 0, 320,  200},   /**< 4:3 = 1.3 / CGA */
									{0, 0, 256,  224},   /**< 256:224 / NES/SNES */
									{0, 0, 256,  192},   /**< 4:3 / TMS9918 1/2, ZX Spectrum, MSX, Nintendo DS */
									{0, 0, 240,  160},   /**< 3:2 / Nintendo Game Boy Advance */
									{0, 0, 160,  152},   /**< 20:19 / Neo Geo Pocket Color */
									{0, 0, 160,  144}    /**< 10:9 / GB, GBC, GG */
};
static const struct {
	int xres;
	int yres;
	...
	/* Generate timings using the 'modeline2fb' with the fbset package. timings based on Matrox Millenium I, 15" monitor. */
...
/**
 * @brief Remove Framebuffer Device
 * @param device
 */
...
static SDL_VideoDevice *FB_CreateDevice(int devindex) {
	...
	if(this) {
		...
		this->hidden = (struct SDL_PrivateVideoData *) SDL_malloc((sizeof *this->hidden));
		this->gl_data = (struct SDL_PrivateGLData *) SDL_malloc((sizeof *this->gl_data));
	}
...
#ifdef HAVE_OSMESA
	this->GL_LoadLibrary = SDL_OSMesa_LoadLibrary;
	this->GL_GetProcAddress = SDL_OSMesa_GetProcAddress;
	this->GL_GetAttribute = SDL_OSMesa_GetAttribute;
	this->GL_MakeCurrent = SDL_OSMesa_MakeCurrent;
	this->GL_SwapBuffers = SDL_OSMesa_SwapBuffers;
#endif
...
#ifdef SDL_DISPLAY_ANYSIZE
	/* For drivers can handle any resolution (eg. OpenDingux IPU) */
...
static int read_fbmodes_mode(FILE *f, struct fb_var_screeninfo *vinfo) {
	...
	SDL_sscanf(line, "geometry %d %d %d %d %d", &vinfo->xres, &vinfo->yres, &vinfo->xres_virtual, &vinfo->yres_virtual, &vinfo->bits_per_pixel);
	if(read_fbmodes_line(f, line, sizeof(line)) == 0) {
		return 0;
	}

	SDL_sscanf(line, "timings %d %d %d %d %d %d %d", &vinfo->pixclock, &vinfo->left_margin, &vinfo->right_margin, &vinfo->upper_margin, &vinfo->lower_margin, &vinfo->hsync_len, &vinfo->vsync_len);
...
static int FB_AddMode(_THIS, int index, unsigned int w, unsigned int h, int check_timings) {
	...
	SDL_modelist[index] = (SDL_Rect **) SDL_realloc(SDL_modelist[index], (1 + next_mode + 1) * sizeof(SDL_Rect *));
...
static int FB_VideoInit(_THIS, SDL_PixelFormat *vformat) {
	...
	mapped_offset = (((long) finfo.smem_start) - (((long) finfo.smem_start) & ~(pagesize - 1)));
	mapped_memlen = finfo.smem_len + mapped_offset;
	mapped_mem = do_mmap(NULL, mapped_memlen, PROT_READ | PROT_WRITE, MAP_SHARED, console_fd, 0);
	...
	if(finfo.accel && finfo.mmio_len) {
		...
		mapped_io = do_mmap(NULL, mapped_iolen, PROT_READ | PROT_WRITE, MAP_SHARED, console_fd, mapped_memlen);
		if(mapped_io == (char *) -1) {
...
static int choose_fbmodes_mode(struct fb_var_screeninfo *vinfo) {
	...
	if(modesdb) {
		...
		while (read_fbmodes_mode(modesdb, &cinfo)) {
			if((vinfo->xres == cinfo.xres && vinfo->yres == cinfo.yres) && (!matched || (vinfo->bits_per_pixel == cinfo.bits_per_pixel))) {
...
static SDL_Surface *FB_SetVideoMode(_THIS, SDL_Surface *current, int width, int height, int bpp, Uint32 flags) {
	...
	if((vinfo.xres != width) || (vinfo.yres != height) || (vinfo.bits_per_pixel != bpp) || (flags & SDL_DOUBLEBUF) || ((flags & SDL_SWIZZLEBGR) != 0 && (vinfo.blue.offset <= vinfo.green.offset)) || ((flags & SDL_SWIZZLEBGR) == 0 && (vinfo.blue.offset >= vinfo.green.offset))) {
		...
		print_vinfo(&vinfo);
#endif
		if(!shadow_fb && ioctl(console_fd, FBIOPUT_VSCREENINFO, &vinfo) < 0) {
			vinfo.yres_virtual = height;
			if(ioctl(console_fd, FBIOPUT_VSCREENINFO, &vinfo) < 0) {
	...
	if(shadow_fb) {
		if(vinfo.bits_per_pixel == 16) {
			blitFunc = (rotate == FBCON_ROTATE_NONE || rotate == FBCON_ROTATE_UD) ? FB_blit16 : FB_blit16blocked;
		} else {
#ifdef DEBUG_FBCON
			...
			SDL_SetError("Using software buffer, but no blitter "
						 "function is available for %d bpp.", vinfo.bits_per_pixel);
			return (NULL);
		}
	}
	...
	surfaces_len = (shadow_fb) ? 0 : (mapped_memlen - (surfaces_mem - mapped_mem));
	...
	if(flags & SDL_DOUBLEBUF) {
		if(vinfo.yres_virtual >= (height * 2)) {
			...
			flip_address[2] = (char *) current->pixels + current->h * current->pitch * 2;
	...
	if ( flags & SDL_OPENGL ) {
		if ( SDL_OSMesa_CreateContext(this,current) < 0 ) {
			return(NULL);
		} else {
...
static void FB_blit16(Uint8 *byte_src_pos, int src_right_delta, int src_down_delta, Uint8 *byte_dst_pos, int dst_linebytes, int width, int height) {
	int w;
...
static void FB_blit16blocked(Uint8 *byte_src_pos, int src_right_delta, int src_down_delta, Uint8 *byte_dst_pos, int dst_linebytes, int width, int height) {
	int w;
	Uint16 *src_pos = (Uint16 *) byte_src_pos;
	Uint16 *dst_pos = (Uint16 *) byte_dst_pos;
	...
	while (height > 0) {
		...
		for (w = width; w > 0; w -= BLOCKSIZE_W) {
			FB_blit16((Uint8 *) src, src_right_delta, src_down_delta, (Uint8 *) dst, dst_linebytes, min(w, BLOCKSIZE_W), min(height, BLOCKSIZE_H));
...
static void FB_DirectUpdate(_THIS, int numrects, SDL_Rect *rects) {
	...
	for (i = 0; i < numrects; i++) {
		...
		src_start = shadow_mem + (sha_y1 * width + sha_x1) * bytes_per_pixel;
		dst_start = mapped_mem + mapped_offset + scr_y1 * physlinebytes + scr_x1 * bytes_per_pixel;

		blitFunc((Uint8 *) src_start, shadow_right_delta, shadow_down_delta, (Uint8 *) dst_start, physlinebytes, scr_x2 - scr_x1, scr_y2 - scr_y1);
	}
}
...
static void FB_SavePalette(_THIS, struct fb_fix_screeninfo *finfo, struct fb_var_screeninfo *vinfo) {
	...
	*/
	if(finfo->visual == FB_VISUAL_DIRECTCOLOR) {
		...
		for (i = 0; i < 256; ++i) {
			new_entries[(0 * 256) + i] = new_entries[(1 * 256) + i] = new_entries[(2 * 256) + i] = (i << 8) | i;
		}
...
/* Note:  If we are terminated, this could be called in the middle of another SDL video routine -- notably UpdateRects. */
static void FB_VideoQuit(_THIS) {
	int i, j;
	...
	if(this->screen) {
		/* Clean up OpenGL */
		this->screen->flags &= ~(SDL_OPENGL|SDL_OPENGLBLIT);
		SDL_OSMesa_Shutdown(this);
		...
		if(((char *) this->screen->pixels >= mapped_mem) && ((char *) this->screen->pixels < (mapped_mem + mapped_memlen))) {
