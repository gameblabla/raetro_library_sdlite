...
void dump_mode(drmModeModeInfo *mode) {
	kmsdrm_dbg_printf("%s %.2f %d %d %d %d %d %d %d %d %d\n", mode->name, mode_vrefresh(mode), mode->hdisplay, mode->hsync_start, mode->hsync_end, mode->htotal, mode->vdisplay, mode->vsync_start, mode->vsync_end, mode->vtotal, mode->clock);
}
...
int save_drm_pipe(_THIS, Uint32 plane, Uint32 crtc, Uint32 enc, drmModeConnector *conn) {
	...
	for (unsigned int i = 0; i < conn->count_modes; i++) {
		...
		if(pipe->factor_w != 1 || pipe->factor_h != 1) {
			KMSDRM_RegisterVidMode(this, conn->modes[i].hdisplay / pipe->factor_w, conn->modes[i].vdisplay / pipe->factor_h);
		}
	}
...
static Uint32 get_prop_ptrs(_THIS, drmModeObjectProperties **props, drmModePropertyRes ***props_info, struct drm_prop_arg *p) {
...
static int find_prop_info_idx(drmModeObjectProperties *props, drmModePropertyRes **props_info, struct drm_prop_arg *p) {
...
static int helper_add_property(_THIS, drmModeAtomicReq *req, struct drm_prop_arg *p) {
	if(helper_find_property(this, p) == 0) {
		...
		SDL_SetError("%s has no property %s.\n", from_mode_object_type(p->obj_type), p->name);
		return 0;
	}
	...
	if(drmModeAtomicAddProperty(req, p->obj_id, p->prop_id, p->value) < 0) {
		kmsdrm_dbg_printf("Failed to set %s property for %s, %s.\n", p->name, from_mode_object_type(p->obj_type), strerror(errno));
	}
