...
int KMSDRM_VideoInit(_THIS, SDL_PixelFormat *vformat) {
		...
		for (int crtc_idx = 0; crtc_idx < res->count_crtcs; crtc_idx++) {
			for (int encoder_idx = 0; encoder_idx < res->count_encoders; encoder_idx++) {
				for (int connector_idx = 0; connector_idx < res->count_connectors; connector_idx++) {
					...
					if(crtc && enc && conn && (plane->possible_crtcs & (1 << crtc_idx)) && (enc->possible_crtcs & (1 << crtc_idx)) && conn->encoder_id == enc->encoder_id && conn->connection == DRM_MODE_CONNECTED && conn->count_modes > 0) {
						// This is a complete, suitable pathway. save it.
						save_drm_pipe(this, plane->plane_id, crtc->crtc_id, enc->encoder_id, conn);
					}
	...
	if(drmModeCreatePropertyBlob(drm_fd, drm_palette, sizeof(drm_palette), &drm_palette_blob_id)) {
...
static int KMSDRM_CreateFramebuffer(_THIS, int idx, Uint32 width, Uint32 height, const drm_color_def *color_def) {
	...
	kmsdrm_dbg_printf("Creating framebuffer %dx%dx%d (%c%c%c%c)\n", width, height, color_def->bpp, color_def->four_cc & 0xFF, (color_def->four_cc >> 8) & 0xFF, (color_def->four_cc >> 16) & 0xFF, color_def->four_cc >> 24);

	// Calculate the necessary information to create the framebuffer
	Uint32 handles[4] = {};
	Uint32 pitches[4] = {};
	Uint32 offsets[4] = {};
	get_framebuffer_args(color_def, req_create->handle, req_create->pitch, height, &handles[0], &pitches[0], &offsets[0]);

	// Create the framebuffer object
	if(drmModeAddFB2(drm_fd, width, height, color_def->four_cc, &handles[0], &pitches[0], &offsets[0], &drm_buffers[idx].buf_id, 0)) {
...
static int KMSDRM_SetCrtcParams(_THIS, drmModeAtomicReqPtr req, Uint32 plane_id, Uint32 crtc_id, int width, int height, int mode_width, int mode_height, int bpp) {
	unsigned int crtc_w = 0;
	unsigned int crtc_h = 0;

	switch (this->hidden->scaling_mode) {
		case DRM_SCALING_MODE_ASPECT_RATIO:
			if(width * mode_height * drm_active_pipe->factor_w > height * mode_width * drm_active_pipe->factor_h) {
				crtc_w = mode_width;
				crtc_h = drm_active_pipe->factor_h * crtc_w * height / (width * drm_active_pipe->factor_w);
			} else {
				crtc_h = mode_height;
				crtc_w = drm_active_pipe->factor_w * crtc_h * width / (height * drm_active_pipe->factor_h);
			}
			break;
		case DRM_SCALING_MODE_INTEGER_SCALED:
			if(width < mode_width / drm_active_pipe->factor_w && height < mode_height / drm_active_pipe->factor_h) {
	...
	if(bpp == 8 && !add_property(this, req, crtc_id, "GAMMA_LUT", 0, drm_palette_blob_id)) {
		return 1;
	}
...
SDL_Surface *KMSDRM_SetVideoMode(_THIS, SDL_Surface *current, int width, int height, int bpp, Uint32 flags) {
	...
	int n_buf = ((flags & SDL_TRIPLEBUF) == SDL_TRIPLEBUF) ? 3 : ((flags & SDL_TRIPLEBUF) == SDL_DOUBLEBUF) ? 2 : 1;
	...
	for (drm_pipe *pipe = drm_first_pipe; pipe; pipe = pipe->next) {
		...
		if(KMSDRM_SetCrtcParams(this, req, pipe->plane, pipe->crtc, width, height, closest_mode->hdisplay, closest_mode->vdisplay, bpp)) {
	...
	// as an intermediary.
	current->flags = SDL_HWSURFACE | (flags & SDL_HWPALETTE) | (flags & SDL_TRIPLEBUF); /* SDL_TRIPLEBUF implies SDL_DOUBLEBUF */
...
static int KMSDRM_TripleBufferingThread(void *d) {
	...
	for (;;) {
		...
		if(KMSDRM_SetCrtcParams(this, req, drm_active_pipe->plane, drm_active_pipe->crtc, this->hidden->w, this->hidden->h, this->hidden->crtc_w, this->hidden->crtc_h, this->hidden->bpp)) {
			fprintf(stderr, "Unable to set CRTC params: %s\n", strerror(errno));
		}

		/* flip display */
		if(!add_property(this, req, drm_active_pipe->plane, "FB_ID", 0, drm_buffers[drm_queued_buffer].buf_id)) {
...
static int KMSDRM_FlipHWSurface(_THIS, SDL_Surface *surface) {
	...
	if((surface->flags & SDL_TRIPLEBUF) == SDL_DOUBLEBUF) {
		...
		if(!add_property(this, req, drm_active_pipe->plane, "FB_ID", 0, drm_buffers[drm_back_buffer].buf_id)) {
			fprintf(stderr, "Unable to set FB_ID property: %s\n", strerror(errno));
		}
...
static void KMSDRM_UpdateRects(_THIS, int num_rects, SDL_Rect *rects) {
	drm_mode_rects *drm_rects;
	drmModeAtomicReqPtr req;
	unsigned int i;
	Uint32 blob_id;
	int ret;
	...
	drm_rects = alloca(num_rects * sizeof(*drm_rects));
	...
	ret = drmModeCreatePropertyBlob(drm_fd, drm_rects, sizeof(*drm_rects) * num_rects, &blob_id);
	if(ret != 0) {
	...
	if(!add_property(this, req, drm_active_pipe->plane, "FB_DAMAGE_CLIPS", 0, blob_id)) {
		fprintf(stderr, "Unable to set FB_DAMAGE_CLIPS property: %s\n", strerror(errno));
	}

	if(!add_property(this, req, drm_active_pipe->plane, "FB_ID", 0, drm_buffers[drm_front_buffer].buf_id)) {
		fprintf(stderr, "Unable to set FB_ID property: %s\n", strerror(errno));
	}
...
int KMSDRM_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color *colors) {
	...
	for (i = firstcolor; i < firstcolor + ncolors; i++) {
		drm_palette[i] = (struct drm_color_lut) {
			.red = colors[i].r << 8, .green = colors[i].g << 8, .blue = colors[i].b << 8,
		};
	}

	if(drmModeCreatePropertyBlob(drm_fd, drm_palette, sizeof(drm_palette), &blob_id)) {
