...
int SDL_CursorInit(Uint32 multithreaded) {
	...
	if(SDL_defcursor == NULL) {
		SDL_defcursor = SDL_CreateCursor(default_cdata, default_cmask, DEFAULT_CWIDTH, DEFAULT_CHEIGHT, DEFAULT_CHOTX, DEFAULT_CHOTY);
...
static void SDL_DrawCursorFast(SDL_Surface *screen, SDL_Rect *area) {
	...
	switch (screen->format->BytesPerPixel) {
		...
		case 1: {
			...
			dst = (Uint8 *) screen->pixels + (SDL_cursor->area.y + area->y) * screen->pitch + SDL_cursor->area.x;
		...
		case 2: {
			...
			dst = (Uint16 *) screen->pixels + (SDL_cursor->area.y + area->y) * screen->pitch / 2 + SDL_cursor->area.x;
		...
		case 3: {
			...
			dst = (Uint8 *) screen->pixels + (SDL_cursor->area.y + area->y) * screen->pitch + SDL_cursor->area.x * 3;
		...
		case 4: {
			...
			dst = (Uint32 *) screen->pixels + (SDL_cursor->area.y + area->y) * screen->pitch / 4 + SDL_cursor->area.x;
