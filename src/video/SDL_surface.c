...
int SDL_SetAlpha(SDL_Surface *surface, Uint32 flag, Uint8 value) {
	 ...
	 * need to invalidate.)
	 */
	if((surface->flags & SDL_HWACCEL) == SDL_HWACCEL || oldflags != surface->flags || (((oldalpha + 1) ^ (value + 1)) & 0x100)) {
		SDL_InvalidateMap(surface->map);
	}
	return (0);
}
...
SDL_Surface *SDL_ConvertSurface(SDL_Surface *surface, SDL_PixelFormat *format, Uint32 flags) {
	...
	if(format->palette != NULL) {
		...
		for (i = 0; i < format->palette->ncolors; ++i) {
			if((format->palette->colors[i].r != 0) || (format->palette->colors[i].g != 0) || (format->palette->colors[i].b != 0)) {
				break;
			}
		}
...
void SDL_FreeSurface(SDL_Surface *surface) {
	...
	if((surface == NULL) || (current_video && ((surface == SDL_ShadowSurface) || (surface == SDL_VideoSurface)))) {
		return;
	}
