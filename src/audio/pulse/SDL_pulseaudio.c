...
#include "SDL_video.h"  /* for SDL_WM_GetCaption(). */
...
static int Audio_Available(void) {
	...
	paspec.rate = 11025;
	paspec.channels = 1;
	connection = SDL_NAME(pa_simple_new)(NULL,                        /* server */
...
static void PULSE_WaitAudio(_THIS) {
	int size;
	while (1) {
		if(SDL_NAME(pa_context_get_state)(context) != PA_CONTEXT_READY || SDL_NAME(pa_stream_get_state)(stream) != PA_STREAM_READY || SDL_NAME(pa_mainloop_iterate)(mainloop, 1, NULL) < 0) {
			this->enabled = 0;
			return;
		}
...
static void PULSE_SetCaption(_THIS, const char *str) {
	...
	if(context != NULL) {
		SDL_NAME(pa_context_set_name)(context, this->hidden->caption, caption_set_complete, 0);
	}
}
...
static int PULSE_OpenAudio(_THIS, SDL_AudioSpec *spec) {
	...
	SDL_NAME(pa_channel_map_init_auto)(&pacmap, spec->channels, PA_CHANNEL_MAP_WAVEEX);
	...
	if(!(context = SDL_NAME(pa_context_new)(mainloop_api, this->hidden->caption))) {
		PULSE_CloseAudio(this);
	...
	stream = SDL_NAME(pa_stream_new)(context, "Simple DirectMedia Layer",  /* stream description */
									 &paspec,                     /* sample format spec */
									 &pacmap                      /* channel map */
	);
	if(stream == NULL) {
	...
	if(SDL_NAME(pa_stream_connect_playback)(stream, NULL, &paattr, flags, NULL, NULL) < 0) {
		PULSE_CloseAudio(this);
