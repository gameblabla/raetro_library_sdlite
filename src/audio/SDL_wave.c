...
static int InitMS_ADPCM(WaveFMT *format, int length) {
	...
	if(rogue_feel + 4 > rogue_feel_end) {
		goto too_short;
	}
...
static int MS_ADPCM_decode(Uint8 **audio_buf, Uint32 *audio_len) {
	...
	*audio_buf = (Uint8 *) SDL_malloc(*audio_len);
	...
	while (encoded_len >= MS_ADPCM_state.wavefmt.blockalign) {
		...
		if(encoded + 7 + (stereo ? 7 : 0) > encoded_end) {
			goto invalid_size;
		}
		...
		if(decoded + 4 + (stereo ? 4 : 0) > decoded_end) {
			goto invalid_size;
		}
		...
		while (samplesleft > 0) {
			if(encoded + 1 > encoded_end) {
				goto invalid_size;
			}
			if(decoded + 4 > decoded_end) {
				goto invalid_size;
			}
...
static int InitIMA_ADPCM(WaveFMT *format, int length) {
	...
	if(rogue_feel + 2 > rogue_feel_end) {
		goto too_short;
	}
...
SDL_AudioSpec *SDL_LoadWAV_RW(SDL_RWops *src, int freesrc, SDL_AudioSpec *spec, Uint8 **audio_buf, Uint32 *audio_len) {
	...
	switch (SDL_SwapLE16(format->encoding)) {
		...
		case IMA_ADPCM_CODE:
			...
			break;
		case MP3_CODE:
			SDL_SetError("MPEG Layer 3 data not supported", SDL_SwapLE16(format->encoding));
			was_error = 1;
			goto done;
		default:
