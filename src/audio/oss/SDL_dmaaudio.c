...
static int Audio_Available(void) {
	...
	if(fd >= 0) {
		...
		if((ioctl(fd, SNDCTL_DSP_GETCAPS, &caps) == 0) && (caps & DSP_CAP_TRIGGER) && (caps & DSP_CAP_MMAP) && (ioctl(fd, SNDCTL_DSP_GETOSPACE, &info) == 0)) {
			available = 1;
		}
		close(fd);
	}
...
static int DMA_ReopenAudio(_THIS, const char *audiodev, int format, int stereo, SDL_AudioSpec *spec) {
	int frag_spec;
	int value;
	...
	if((ioctl(audio_fd, SNDCTL_DSP_STEREO, &stereo) < 0) || (value != stereo)) {
...
static int DMA_OpenAudio(_THIS, SDL_AudioSpec *spec) {
	...
	format = 0;
	for (test_format = SDL_FirstAudioFormat(spec->format); !format && test_format;) {
#ifdef DEBUG_AUDIO
	...
	dma_buf = (Uint8 *) mmap(NULL, dma_len, PROT_WRITE, MAP_SHARED, audio_fd, 0);
