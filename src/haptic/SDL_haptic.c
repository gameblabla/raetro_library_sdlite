...
void SDL_HapticClose(SDL_Haptic *haptic) {
	...
	for (i = 0; SDL_haptics[i]; ++i) {
		if(haptic == SDL_haptics[i]) {
			SDL_haptics[i] = NULL;
			SDL_memcpy(&SDL_haptics[i], &SDL_haptics[i + 1], (SDL_numhaptics - i) * sizeof(haptic));
			break;
		}
	}
 ...
 */
int SDL_HapticMinDuration(SDL_Haptic *haptic) {
	if(!ValidHaptic(haptic)) {
		return -1;
	}
 ...
 * Palm Edit: Returns minimum duration of vibration - which is 0ms;
 */
int SDL_HapticMaxDuration(SDL_Haptic *haptic) {
	if(!ValidHaptic(haptic)) {
		return -1;
	}
