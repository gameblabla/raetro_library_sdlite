...
int SDL_SYS_HapticOpen(SDL_Haptic *haptic) {
	...
	if(fd < 0) {
		SDL_SetError("Haptic: Unable to open %s: %s", item->fname, strerror(errno));
		return -1;
	}
...
int SDL_SYS_HapticOpenFromJoystick(SDL_Haptic *haptic, SDL_Joystick *joystick) {
	...
	if(fd < 0) {
		SDL_SetError("Haptic: Unable to open %s: %s", joystick->hwdata->fname, strerror(errno));
		return -1;
	}
	ret = SDL_SYS_HapticOpenFromFD(haptic, fd); /* Already closes on error. */
	if(ret < 0) {
...
static int SDL_SYS_ToDirection(Uint16 *dest, SDL_HapticDirection *src) {
	...
	switch (src->type) {
		case SDL_HAPTIC_POLAR:
			/**
			 * Linux directions start from south. (and range from 0 to 0xFFFF)
...
void SDL_SYS_HapticDestroyEffect(SDL_Haptic *haptic, struct haptic_effect *effect) {
	if(ioctl(haptic->hwdata->fd, EVIOCRMFF, effect->hweffect->effect.id) < 0) {
		SDL_SetError("Haptic: Error removing the effect from the device: %s", strerror(errno));
	}
	SDL_free(effect->hweffect);
	effect->hweffect = NULL;
}
