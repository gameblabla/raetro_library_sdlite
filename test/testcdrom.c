	...
	for (; argv[i]; ++i) {
		if(strcmp(argv[i], "-status") == 0) {
			...
			if(CD_INDRIVE(SDL_CDStatus(cdrom))) {
				if(SDL_CDPlayTracks(cdrom, strack, sframe, ntrack, nframe) < 0) {
					fprintf(stderr, "Couldn't play tracks %d/%d for %d/%d: %s\n", strack, sframe, ntrack, nframe, SDL_GetError());
				}
			} else {
		...
		} else if((strcmp(argv[i], "-sleep") == 0) && (argv[i + 1] && isdigit(argv[i + 1][0]))) {
