...
void DrawPoints(SDL_WindowID window) {
	int i;
	int x, y;
	...
	for (i = 0; i < num_objects * 4; ++i) {
		...
		SDL_SetRenderDrawColor(255, (Uint8) current_color, (Uint8) current_color, (Uint8) current_alpha);
...
void DrawLines(SDL_WindowID window) {
	int i;
	int x1, y1, x2, y2;
	int window_w, window_h;
...
static void DrawRects(SDL_WindowID window) {
	int i;
	int window_w, window_h;
...
static void DrawRectLineIntersections(SDL_WindowID window) {
...
static void DrawRectRectIntersections(SDL_WindowID window) {
	int i, j;
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc;) {
		...
		if(consumed < 0) {
			fprintf(stderr, "Usage: %s %s [--blend none|mask|blend|add|mod] [--cyclecolor] [--cyclealpha]\n", argv[0], CommonUsage(state));
			return 1;
		}
		i += consumed;
	}
					...
					switch (event.key.keysym.sym) {
						case 'l':
							if(event.key.keysym.mod & KMOD_SHIFT) {
								...
								add_line(rand() % 640, rand() % 480, rand() % 640, rand() % 480);
							}
							break;
						case 'r':
							if(event.key.keysym.mod & KMOD_SHIFT) {
								...
								add_rect(rand() % 640, rand() % 480, rand() % 640, rand() % 480);
							}
							break;
					}
					break;
