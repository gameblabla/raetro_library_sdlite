...
int SDLTest_RunSuites(SDLTest_TestSuiteReference *testSuites[], const char *userRunSeed, Uint64 userExecKey, const char *filter, int testIterations) {
	...
	while (testSuites[suiteCounter]) {
		...
		if(suiteFilter == 1 && suiteFilterName != NULL && testSuite->name != NULL && SDL_strcmp(suiteFilterName, testSuite->name) != 0) {
			/* Skip suite */
			...
			while (testSuite->testCases[testCounter]) {
				...
				if(testFilter == 1 && testFilterName != NULL && testCase->name != NULL && SDL_strcmp(testFilterName, testCase->name) != 0) {
					/* Skip test */
					...
					if(testCase->description != NULL && testCase->description[0] != '\0') {
						SDLTest_Log("Test Description: '%s'", (testCase->description) ? testCase->description : SDLTEST_INVALID_NAME_FORMAT);
					}
					...
					while (iterationCounter < testIterations) {
						...
						SDLTest_Log("Test Iteration %i: execKey %"
						SDL_PRIu64, iterationCounter, execKey);
