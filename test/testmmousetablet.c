		...
		if(SDL_PollEvent(&event) == 0) {
			...
			switch (event.type) {
				case SDL_MOUSEMOTION:
					...
					pressure_max: %d pressure_min: %d current cursor:%d\n", event.motion.which, event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel, event.motion.pressure, event.motion.pressure_max, event.motion.pressure_min, event.motion.cursor);
					break;
