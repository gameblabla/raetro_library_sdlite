...
Uint8 small_cursor_data[11] = {
	0x00,0x18,0x08,0x38,0x44,0x54,
	0x44,0x38,0x20,0x20,0x00
};
Uint8 small_cursor_mask[11] = {
	0x3C,0x3C,0x3C,0x7C,0xC6,0xC6,
	0xC6,0x7C,0x78,0x70,0x70
};
...
int main(int argc, char *argv[]) {
	...
	cursor[0] = SDL_CreateCursor((Uint8 *) cursor_data, (Uint8 *) cursor_mask, 16, 16, 8, 8);
	if(cursor[0] == NULL) {
	...
	cursor[2] = SDL_CreateCursor(small_cursor_data, small_cursor_mask, 8, 11, 3, 5);
	if(cursor[2] == NULL) {
		fprintf(stderr, "Couldn't initialize test cursor: %s\n", SDL_GetError());
		SDL_Quit();
		return (1);
