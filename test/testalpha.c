...
SDL_Surface *CreateLight(int radius) {
	...
	light = SDL_CreateRGBSurface(SDL_SWSURFACE, 2 * radius, 2 * radius, 32, 0xFF000000, 0x00FF0000, 0x0000FF00, alphamask);
	if(light == NULL) {
...
int LoadSprite(SDL_Surface *screen, char *file) {
	...
	backing = SDL_CreateRGBSurface(SDL_SWSURFACE, sprite->w, sprite->h, 8, 0, 0, 0, 0);
	if(backing == NULL) {
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; argv[i]; ++i) {
		if(strcmp(argv[i], "-bpp") == 0) {
			...
			fprintf(stderr, "Usage: %s [-width N] [-height N] [-bpp N] [-warp] [-hw] [-fullscreen]\n", argv[0]);
			quit(1);
		}
	}
