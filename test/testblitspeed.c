...
static void __append_sdl_surface_flag(SDL_Surface *_surface, char *str, size_t strsize, Uint32 flag, const char *flagstr) {
...
static void output_surface_details(const char *name, SDL_Surface *surface) {
	...
	if(surface == NULL) {
		...
		printf("  red bits   : 0x%08X mask, %d shift, %d loss\n", (int) surface->format->Rmask, (int) surface->format->Rshift, (int) surface->format->Rloss);
		printf("  green bits : 0x%08X mask, %d shift, %d loss\n", (int) surface->format->Gmask, (int) surface->format->Gshift, (int) surface->format->Gloss);
		printf("  blue bits  : 0x%08X mask, %d shift, %d loss\n", (int) surface->format->Bmask, (int) surface->format->Bshift, (int) surface->format->Bloss);
		printf("  alpha bits : 0x%08X mask, %d shift, %d loss\n", (int) surface->format->Amask, (int) surface->format->Ashift, (int) surface->format->Aloss);
...
static int setup_test(int argc, char **argv) {
	...
	src = SDL_CreateRGBSurface(srcflags, srcw, srch, srcbpp, srcrmask, srcgmask, srcbmask, srcamask);
	if(src == NULL) {
...
static void test_blit_speed(void) {
	...
	printf("%d blits took %d ms (%d fps).\n", (int) iterations, (int) elasped, (int) (((float) iterations) / (((float) elasped) / 1000.0f)));
}
