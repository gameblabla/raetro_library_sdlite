...
void DrawPict(SDL_Surface *screen, char *bmpfile, int speedy, int flip, int nofade) {
	SDL_Surface *picture;
	...
	if(picture->format->palette) {
		...
		memcpy(colors, picture->format->palette->colors, ncolors * sizeof(SDL_Color));
	} else {
		int r, g, b;
	...
	if(!SDL_SetColors(screen, colors, 0, ncolors) && (screen->format->palette != NULL)) {
