 ...
 */
static SDL_Color wavemap[] = {{0,   2,   103},
							  {0,   7,   110},
							  {0,   13,  117},
							  {0,   19,  125},
							  {0,   25,  133},
...
static SDL_Surface *make_bg(SDL_Surface *screen, int startcol) {
	int i;
	SDL_Surface *bg = SDL_CreateRGBSurface(SDL_SWSURFACE, screen->w, screen->h, 8, 0, 0, 0, 0);
	if(!bg) {
...
static SDL_Surface *hflip(SDL_Surface *s) {
	int i;
	SDL_Surface *z = SDL_CreateRGBSurface(SDL_SWSURFACE, s->w, s->h, 8, 0, 0, 0, 0);
	/* copy palette */
	SDL_SetColors(z, s->format->palette->colors, 0, s->format->palette->ncolors);
...
int main(int argc, char **argv) {
	...
	while (--argc) {
		...
		if(strcmp(*argv, "-hw") == 0) {
			...
			fprintf(stderr, "usage: testpalette "
	...
	SDL_SetColorKey(boat[0], SDL_SRCCOLORKEY | SDL_RLEACCEL, SDL_MapRGB(boat[0]->format, 0xff, 0x00, 0xff));
	boatcols = boat[0]->format->palette->ncolors;
	boat[1] = hflip(boat[0]);
	SDL_SetColorKey(boat[1], SDL_SRCCOLORKEY | SDL_RLEACCEL, SDL_MapRGB(boat[1]->format, 0xff, 0x00, 0xff));
	 ...
	 */
	SDL_SetPalette(screen, SDL_LOGPAL, boat[0]->format->palette->colors, 0, boatcols);
	...
	do {
		...
		while (SDL_PollEvent(&e)) {
			if(e.type == SDL_KEYDOWN || e.type == SDL_QUIT || e.type == SDL_MOUSEBUTTONDOWN) {
				if(fade_dir < 0) {
					fade_level = 0;
				}
				fade_dir = -1;
		...
		if(fade_dir) {
			...
			if(gamma_fade) {
				...
				memcpy(cmap, screen->format->palette->colors, boatcols * sizeof(SDL_Color));
